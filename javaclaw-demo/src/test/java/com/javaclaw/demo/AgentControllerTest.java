...
class AgentControllerTest {
    ...
    void startTask_returnsSubmissionId() {
        ChatClient chatClient = mockChatClient(() ->
                """
                {"thought": "Done", "action": "finish", "input": "result"}
                """);
    ...
    void asyncApprovalFlow_approveViaEndpoint() throws Exception {
        toolRegistry.register(new ToolDefinition("sensitive", "Sensitive op", input -> "executed"));
        ...
        ChatClient chatClient = mockChatClient(() -> {
            ...
            if (call == 0) {
                ...
                return """
                        {"thought": "Done", "action": "finish", "input": "Completed after approval"}
                        """;
            }
        });
    ...
    void asyncApprovalFlow_rejectViEndpoint() throws Exception {
        toolRegistry.register(new ToolDefinition("sensitive", "Sensitive op", input -> "should not run"));
